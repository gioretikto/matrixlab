...
void calculate(struct m *matrix, struct operator *op){
    ...
    {        
        ...
        if(op->symbol[i] == '*' && op->symbol[i+1] == 'd'){		/* Matrix1 * det(Maitrix2) or number * det(Maitrix2)*/
                 ...
                 if(op->symbol[i+2] == '?')
              	  	...
              	  	tmp = add(&matrix[i+1], &matrix[i+2], op->symbol[i+2]);
          		 ...
          		 tmp = multiply(&matrix[i], &matrix[i+1]);  /* Multiplication of Scalar per matrix */
                 copy_matrix(&matrix[i+1], &tmp);
